...
{	
	...
	public void start(Stage my_stage) 
	{
		...
		Scene my_scene = new Scene(root_node, 400, 300);
		...
		track_slider.valueProperty().addListener(new ChangeListener<Number>() 
		{
			public void changed(ObservableValue<? extends Number> ov, Number old_val, Number new_val) 
			{
				if(player != null)
					player.seek(Duration.millis(ov.getValue().doubleValue()));
			}
		});
		...
		shuffle_tracks.setOnAction(new EventHandler<ActionEvent>() 
		{
			public void handle(ActionEvent event) 
			{
				shuffle();
		...
		root_node.getChildren().addAll(button_layout, volume_layout,
				...
				next_buttons_layout, shuffle_buttons);
	...
	private void select_track(int index) 
	{
		if(!track_list.isEmpty()) 
		{
			...
			else if(sel_model.getSelectedIndex() == 0)
			{
				sel_model.select(track_list.size() - 1);
			}
			else if(sel_model.getSelectedIndex() == track_list.size() - 1) 
			{
				sel_model.select(0);
			}
			track = file_list.get(sel_model.getSelectedItem());
	...
	{
		Collections.shuffle(track_list);
	}
